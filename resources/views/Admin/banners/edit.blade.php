    ...
    <div class="admin-container">
        ...
        <form action="{{ route('admin.banners.update', $banner->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input type="text" id="link" name="link" value="{{ $banner->link }}" required>
            </div>
            <div class="form-group">
                <label for="is_published">Xuất Bản</label>
                <select id="is_published" name="is_published" required>
                    <option value="1" {{ $banner->is_published == 1 ? 'selected' : '' }}>Có</option>
                    <option value="0" {{ $banner->is_published == 0 ? 'selected' : '' }}>Không</option>
                </select>
            </div>
            <div class="form-group">
                <label for="description">Mô Tả</label>
                <textarea id="description" name="description" required>{{ $banner->description }}</textarea>
            </div>
            <div class="form-group">
